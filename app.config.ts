...
// This app's release build merges a large dex set (maplibre + reanimated +
// hermes + many native modules). The Expo template's default Gradle heap of
// -Xmx2048m OOMs the D8 dexer on the EAS build path ("java.lang.OutOfMemoryError:
// Java heap space" at :app:mergeDexRelease / :app:packageRelease). Raise the
// Gradle daemon heap to 4 GB. android/ is gitignored and regenerated by prebuild
// on every build, so this must live in config — editing android/gradle.properties
// directly would be overwritten.
//
// Metaspace is 2 GB rather than the 512 MB this started with: under SDK 57's AGP,
// `lintVitalAnalyzeRelease` exhausted 512 MB and failed with a bare "> Metaspace"
// across half the native modules (datetimepicker, maplibre, async-storage,
// keyboard-controller...). It only bites release builds — assembleDebug does not
// run lintVital — so it surfaces on EAS preview/production, not in dev.
...
export default (): ExpoConfig => {
  ...
  // Apply the inline mods here rather than via the typed `plugins` array
  // (ExpoConfig['plugins'] does not accept inline function plugins). Expo runs
  // the attached mods during prebuild.
  return withDisabledAndroidBackup(withGradleHeap(config)) as ExpoConfig;
};
